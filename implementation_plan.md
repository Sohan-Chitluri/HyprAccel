...
- **`NL-4WD` | 4WD / Mobile Robotics**: `Complete` for Phase 2 `EncoderInput`, `MotorOutput`, `WheelSpeed`, `DifferentialDrive`. Uses new `hyp_encoder_read` SDK primitive (SDK-T5) with ESP32 PCNT backend; `MotorOutput` reuses `hyp_actuator_write`; `WheelSpeed` and `DifferentialDrive` are pure computation nodes; graph schema, palette, inspector, ports, validation, deterministic C codegen, and tests complete.
